class Mascaras {
	...
	static cargapersonaje() {
		...
		document.getElementById("dados").innerHTML = "<img src='imagenes/d6.png' onclick='RolTelegram.tiradado(6)' alt='Dado d6' class='mano'/>";
		
		for (indiceatr=0; indiceatr< personaje.atributos.length; indiceatr++) {
			seccion = "<div class='w3-container w3-blue w3-center'><p class='mano' onclick='Mascaras.tiradaatr(" + indiceatr +")'><strong>" + personaje.atributos[indiceatr].nombre + "</strong> " + personaje.atributos[indiceatr].valor + "</p></div>"
			seccion += "<table class='w3-table  w3-striped w3-border'>";
			
			
			for (indice=0; indice< personaje.atributos[indiceatr].pericias.length; indice++) {
				var funcion="Mascaras.tiradapericia(" + indiceatr + ", " + indice + ")";
				if ( personaje.atributos[indiceatr].pericias[indice].nombre == "Combate cuerpo a cuerpo")
					funcion="Mascaras.tiradaCombate(1)";
				else if ( personaje.atributos[indiceatr].pericias[indice].nombre == "Combate sin armas")
					funcion="Mascaras.tiradaCombate(2)";
				else if ( personaje.atributos[indiceatr].pericias[indice].nombre == "Combate a distancia" )
					funcion="Mascaras.tiradaCombate(3)";
				
				seccion += "<tr><td><span class='mano' onclick='" + funcion + "' >" + personaje.atributos[indiceatr].pericias[indice].nombre + "</span></td><td>";
				if ( personaje.atributos[indiceatr].pericias[indice].valor == true ) {
		...
		for (indice=0; indice<personaje.caracteristicas.length; indice++) {
			seccion += "<tr><td>" + personaje.caracteristicas[indice].nombre + "</td><td><span class='mano' onclick='Mascaras.cambiacaracteristica(" + indice + ")' >" + personaje.caracteristicas[indice].valor + "</span></td><tr>";
		}
		seccion += "</table>";
		...
		seccion += "<tr><td>" + personaje.armaCaC.nombre + "</td><td>(CaC)</td><td>" + personaje.armaCaC.daño + "</td></tr>";
		seccion += "<tr><td>" + personaje.armaAD.nombre + "</td><td>(AD)</td><td>" + personaje.armaAD.daño + "</td></tr>";
		...
		for (indice=0; indice<personaje.talentos.length; indice++) {
			seccion += "<b>" + personaje.talentos[indice].nombre + "</b>: " + personaje.talentos[indice].descripcion + "<br/>";
		}
	...
	static modificacaracteristica(indice) {
		personaje.caracteristicas[indice].valor = document.getElementById('vcaracteristica').value;
	...
	static cambiacaracteristica(indice) {
		document.getElementById('nbcaracteristica').innerHTML = "<label id='nbcaracteristica'>Característica " + personaje.caracteristicas[indice].nombre + "</label>";
		document.getElementById('btncaracteristica').innerHTML = "<button class='w3-btn w3-blue' id='btncaracteristica' onclick='Mascaras.modificacaracteristica(" + indice + ")'>Modifica</button>";
		...
		document.getElementById('vcaracteristica').value = personaje.caracteristicas[indice].valor;
