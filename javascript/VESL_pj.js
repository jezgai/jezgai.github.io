class PJSL extends PJBase {
	...
	calculaPV() {
		...
		if ( this._nivel >= 6 )
			niv = 5;
		this._pv = this._objClase.pv(niv, this.modifAtributo(this._atributos[atributos.atributoMod("CON")]));
		var i=0;
		for (i=niv+1; i<= this._nivel; i++) {
			this._pv += this._objClase.pgniv6;
		}
	}
	...
	modifAtributo(valorAtributo) {
		return Atributos.modifmas(valorAtributo); //Atributos.modif(valorAtributo);
	}
	...
	genera() {
		...
		var raza = razasSL.raza(this._raza);
		this._objClase = clasesSL.clase(this._clase);
		...
		this._nombre = "PJ"; //raza.nombrepj();
		this._mov = raza.mov;
		this._trasfondo = trasfondos.trasfondo();
		
		var ital = 0;
		this._talentos = raza.talentospj(this._nivel);
	...
	rellenaCamposPDF() {
		return this.rellenaCamposPDFExpandido();
	}
